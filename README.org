...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar elamdf/fixed-width-font "JetBrainsMono Nerd Font Mono"
  ...
  ;;)
  ;; make shell output not take forever to display
...
#+BEGIN_SRC emacs-lisp
  ...
  (require 'obsidian-helpers)                         ; personal helpers
...
#+BEGIN_SRC emacs-lisp
    ...
    (dolist (hook '(org-mode-hook term-mode-hook shell-mode-hook eshell-mode-hook))
      ...
      (global-set-key (kbd "C-M-<tab>") #'eglot-code-actions)



  (global-set-key (kbd "C-M-g") 'elamdf/switch-to-minibuffer-window)
...
#+BEGIN_SRC emacs-lisp
            ...
            (use-package markdown-mode
              ...
              :init (progn (setq markdown-command "multimarkdown") (setq-default markdown-hide-markup nil))
    ...
    ;; copy region as fontified rich text, useful for copying into keynote
  (require 'highlight2clipboard)
  ;; not on my default, makes killing/yanking slow
  (highlight2clipboard-mode 0)
...
#+begin_src emacs-lisp
  ...
  (setq markdown-max-image-size (cons 1000 1000))

  (define-key markdown-mode-map (kbd "C-c C-a w") #'elamdf/insert-relative-wiki-link)

  (define-key markdown-mode-map (kbd "C-c C-a h") #'elamdf/insert-relative-wiki-header-link)

  (define-key markdown-mode-map (kbd "C-c C-o") #'elamdf/markdown-follow-thing-at-point)
  (define-key markdown-mode-map (kbd "C-c C-x TAB") #'elamdf/markdown-toggle-inline-images)
  (setq markdown-link-space-sub-char " ") ;; obsidian doesn't do space substitution
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq magit-bury-buffer-function 'magit-restore-window-configuration)
    ...
    (add-hook 'projectile-after-switch-project-hook
              #'(lambda () (if (magit-toplevel) (magit-status) (projectile-find-file))))
      (use-package ripgrep)
      (use-package rg)
    ;; get ansi color in compilation output
  (require 'ansi-color)
  (defun elamdf/compilation-ansi-color ()
    (let ((inhibit-read-only t))
      (ansi-color-apply-on-region compilation-filter-start (point))))
  (add-hook 'compilation-filter-hook #'elamdf/compilation-ansi-color)
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    :ensure t
    :hook ((( scala-mode python-mode rust-mode tuareg-mode caml-mode c-mode-hook c++-mode-hook c-or-c++-mode-hook )
            . eglot-ensure)
  ...
  ;; checks
  ;; > CR elamdf for elamdf: make C-c ! n/p only go to errors
  ...
  (use-package typst-preview
    :load-path "lisp/typst-preview.el")
  (use-package opam-switch-mode
    :ensure t
    :hook
    ((coq-mode tuareg-mode) . opam-switch-mode))
  (use-package proof-general)
  (push '("\\.v\\'" . verilog-mode) auto-mode-alist)
...
(setq python-shell-interpreter "python3")
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '((python-mode python-ts-mode) . ("python3" "-m" "pylsp"))))
...
#+begin_src emacs-lisp
  ...
  ;; (add-to-list 'eglot-server-programs
  ;;              '((verilog-mode verilog-ts-mode verilog-ext-mode) . ("svlangserver")))
...
#+RESULTS:
| verilog-ext-eglot-svlangserver-configuration | verilog-ext-typedef-proj-hook | hs-minor-mode | verilog-ext-mode |
...
#+RESULTS:
| flycheck-rust-setup | flycheck-mode--set-explicitly | doom-modeline-update-flycheck |

* pendeo lsp stuff
#+begin_src emacs-lisp
  (add-to-list 'auto-mode-alist '("\\.p\\'" . pendeo-mode))

#+end_src


* Combobulate for AST manipulation
#+begin_src emacs-lisp
(use-package combobulate
  :load-path (lambda ()
               (expand-file-name "plugins/combobulate" user-emacs-directory))
  :custom
...
#+RESULTS:
| combobulate-mode | yas-minor-mode | rainbow-delimiters-mode |
...
#+RESULTS:
| balance-parens | inline-cr-mode | elamdf/markdown-no-comment-start |
...
( setq codex-terminal-backend 'ghostel)

#+end_src

#+RESULTS:
: (keymap (9 . codex-queue-followup) (69 . codex-edit-previous-message) (73 . codex-send-image) (111 . codex-send-buffer-file) (77 . codex-cycle-permissions) (51 . codex-send-3) (50 . codex-send-2) (49 . codex-send-1) (122 . codex-toggle-read-only-mode) (121 . codex-send-return) (120 . codex-send-command-with-context) (116 . codex-toggle) (115 . codex-send-command) (114 . codex-send-region) (110 . codex-send-escape) (109 . codex-transient) (108 . codex-redraw) (75 . codex-kill-all) (107 . codex-kill) (101 . codex-fix-error-at-point) (100 . codex-start-in-directory) (105 . codex-new-instance) (102 . codex-fork) (82 . codex-resume) (99 . codex) (66 . codex-select-buffer) (98 . codex-switch-to-buffer) (47 . codex-slash-commands))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; this seems to be required to make things normal sized lol
...
#+begin_src emacs-lisp
 ...
 (setq org-todo-keywords '((sequence "TODO(t)" "WAIT(w@/!)" "WATCH(v)" "READ(r)" "|" "DONE(d!)" "CANCELED(c@)")))
  ...
  ;; no longer necessary?
  ;; (use-package ox-md) ; reveal js presentations!n
  ;; (
...
#+BEGIN_SRC emacs-lisp
    (require 'inline-cr)
      (require 'bismuth-magit)
    (require 'brain)
    ;; Enable inline‑cr in common prose modes
    (global-set-key (kbd "M-;") #'inline-cr-maybe-toggle-active)
    (dolist (hook '(markdown-mode-hook org-mode-hook c-mode-hook rust-mode-hook rust-conf-mode))
      (add-hook hook #'inline-cr-mode))
  ;; kinda kills perf bc loads synchrononusly with status
  ;; (add-hook 'magit-status-sections-hook #'bismuth-magit-insert-actionables t)

#+END_SRC

#+RESULTS:
| magit-insert-error-header | magit-insert-diff-filter-header | magit-insert-head-branch-header | magit-insert-upstream-branch-header | magit-insert-push-branch-header | magit-insert-tags-header | bismuth-magit-insert-actionables |
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package olivetti) ; nice formatting for text
          (setq elfeed-feeds '("https://people.csail.mit.edu/rachit/post/atom.xml"
                               ...
                               "https://mtlcad.mit.edu/rss.xml"
                               "https://elam.day/post/atom.xml"
                               "https://stng.substack.com/feed"
                               "https://roonscape.ai/feed"
                               "https://www.julian.ac/feeds/all.rss.xml"
                               "https://mcyoung.xyz/feed"
                               "https://www.thonking.ai/feed"
...
=rust-analyzer=
typst
opam install utop merlin ocamlformat
