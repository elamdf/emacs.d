...
(defcustom highlight2clipboard-python-program
  (or (seq-find #'file-executable-p
                '("/opt/anaconda3/bin/python3"
                  "/Users/elamdf/.pyenv/shims/python3"))
      (executable-find "python3")
      "python3")
  "Python executable used by the macOS clipboard helper."
  :type 'string
  :group 'highlight2clipboard)
...
(defun highlight2clipboard-copy-to-clipboard (text &optional _ignored)
  ...
  (let ((file-name-html (concat highlight2clipboard--temp-file-base-name
                                ".html"))
        (file-name-text (concat highlight2clipboard--temp-file-base-name
                                ".txt")))
    (with-temp-file file-name-text
...
(defun highlight2clipboard--add-html-to-clipboard-osx (file-name text-file-name)
  ...
  (let ((default-directory exec-directory))
    (with-temp-buffer
      (unless (zerop
               (call-process
                highlight2clipboard-python-program
                nil
                t
                nil
                ...
                file-name
                text-file-name))
        (user-error "Failed to add HTML to macOS clipboard: %s"
                    (string-trim (buffer-string)))))))


(defun highlight2clipboard--add-html-to-clipboard-w32 (file-name _text-file-name)
  ;; In case the directory in `default-directory' doesn't exist, Emacs
  ;; fail to spawn the process.
  (let ((default-directory exec-directory))
    (call-process
